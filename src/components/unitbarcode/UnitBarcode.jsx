...
import UnitBarcodeForm from "./UnitBarcodeForm";
...
class UnitBarcode extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            dataLoaded: false,
    ...
    handleOnChange = name => event => {
        let val = (name === 'item' || name === 'unitsetLine') ? event : event.target.value;
    ...
    checkIsNull(valueData) {
        ...
        if (valueData.barcode === null || valueData.barcode === ''
            || !valueData.item || valueData.item.value === 0
            || !valueData.unitsetLine || valueData.unitsetLine.value === 0)
             {
            this.setState(Object.assign({}, this.state, {
                formData: Object.assign({}, this.state.formData, {
                    errors: Object.assign({}, this.state.formData.errors, {
                        barcodeIsNull: valueData.barcode === null || valueData.barcode === '',
                        unitsetLineIsNull:  !valueData.unitsetLine || valueData.unitsetLine.value === 0,
                        itemIsNull:  !valueData.item || valueData.item.value === 0,
                    })
                }),
...
export default withStyles(styles)(connect(null, null)(UnitBarcode));
