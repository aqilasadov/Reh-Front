...
import KassaBankDefaultForm from "./KassaBankDefaultForm";
...
class KassaBankDefault extends CoreComponent {
    constructor(props) {
        super(props);
        this.state = {
            dataLoaded: false,
    ...
    handleOnChange = name => event => {
        let val;
        if (name === 'kassaType' || name === 'kassaBank' || name === 'kbType' || name === 'client' || name === 'faktura')
            val = event;
    ...
    handleTableAction = (id, action) => {
        ...
        if (action === 'add') {
            this.getMethod(BACKEND_URL + '/api/kassaitems/generatecode', ()=>{}, err => alert(err), data => {
...
export default withStyles(styles)(connect(null, null)(KassaBankDefault));
