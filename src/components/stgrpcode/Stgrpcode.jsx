...
import StgrpcodeForm from "./StgrpcodeForm";
...
class Stgrpcode extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            dataLoaded: false,
    ...
    handleOnChange = name => data => {
        const val = (name === 'branch') ? data : data.target.value;
            ...
            .then(
                (value) => {
                    if (!isEmpty(value) && value.success === true) {
                        ...
                        const rows = this.updateTableRows(valueData.value, value.data);
                        // console.log('rows: ', rows);
                        this.setState(Object.assign({}, this.state, {
                            tableData: rows,
                            snackBar: Object.assign({}, this.state.snackBar, {
                                show: true,
                                message: ifNull(valueData, 0).value === 0 ? 'Операция успешно завершена' : 'Информация успешно изменена',
                                variant: 'success'
                            }),
    ...
    updateTableRows = (id, item) => {
        ...
        const insertIndex = id === 0 ? 0 : rows.findIndex(item => item.value === id);
    ...
    delete() {
        ...
        avrFetch(DELETE_URL.replace("{id}", id), {method: "DELETE",})
            ...
            .then(
                (value) => {
                    if (!isEmpty(value) && value.success === true) {
                        const rows = this.state.tableData.slice().filter(item => item.value !== id);
                        this.setState(Object.assign({}, this.state, {
                            tableData: rows,
                            snackBar: Object.assign({}, this.state.snackBar, {
                                show: true,
...
export default withStyles(styles)(connect(null, null)(Stgrpcode));
