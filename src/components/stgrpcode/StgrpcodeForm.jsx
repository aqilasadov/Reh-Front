...
import {CompData, ValueData} from "./StgrpcodeHelper";
        ...
        <Fragment>
            <Dialog
                ...
                <DialogTitle id="customized-dialog-title" onClose={() => dialogAction('disagree')}>
                    {valueData.value === 0 ? 'Новый' : 'Редактировать'}
                </DialogTitle>
                <DialogContent>
                    ...
                    <Grid container spacing={1}>
                        <Grid item xs={12} sm={12}>
                            <CustomInput
                                id="label"
                                labelText={'Название'}
                                error={errors.labelIsNull}
                                formControlProps={{fullWidth: true}}
                                inputProps={{
                                    onChange: onChange('label'),
                                    name: 'label',
                                    value: ifNull(valueData.label)
                                }}
                            />
                        </Grid>
                ...
                <DialogActions>
                    ...
                    <Button onClick={() => dialogAction('agree')} color="primary"
                            autoFocus> {valueData.value === 0 ? 'Сохранить' : 'Изменять'}</Button>
                </DialogActions>
            </Dialog>
        </Fragment>
    );
