...
import {
    ...
    DELETE_URL,
    Errors,
    FilterData, GENERATE_NEW_CODE_URL,
    INSERT_URL,
    LOAD_BY_ID_URL,
    LOAD_COMP_DATA_URL,
    LOAD_DATA_URL,
    MODULE_NAME,
    ValueData
} from "./KassaBankClientDefaultHelper";
...
import KassaBankClientDefaultForm from "./KassaBankClientDefaultForm";
...
class KassaBankClientDefault extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            dataLoaded: false,
    ...
    handleOnChange = name => event => {
        let val;
        if (name === 'client' || name === 'clientType')
            val = event;


        else if (name === 'operDate')
    ...
    generateNewCode() {
        avrFetch(GENERATE_NEW_CODE_URL)
            .then(validateResponse)
            .then(readResponseAsJSON)
            .then(
                (value) => {
...
export default withStyles(styles)(connect(null, null)(KassaBankClientDefault));
