...
import ItemCardAmountForm from "./Trans5Form";
...
import TransferFilter from "./KassaFilterMain";
import DataGrid, {
    Column,
    ColumnChooser,
    ColumnFixing,
...
class Trans5 extends CoreComponent {
    constructor(props) {
        super(props);
        this.state = {
            processing: false,
    ...
    handleOnChange = name => event => {
        ...
        let yekundeger=0;
        let qazanc=0;
        if (name === 'fakturaType' || name === 'item' || name === 'unitsetGrp' || name === 'client' || name === 'branch' || name === 'currency'|| name === 'currencyKassa')
            val = event;
        ...
        {
           // val = Date(0).setUTCSeconds(moment(event).unix());
            val = moment(event).unix();
            console.log("val 55",val);
        }
        else val = event.target.value;

console.log("evet.value",val);
        // if (name === 'mebleg') {
...
export default withStyles(styles)(connect(null, null)(Trans5));
