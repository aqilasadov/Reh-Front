...
import StgrpcodeForm from "./StgrpotherForm";
...
class Stgrpcode extends React.Component {
            ...
            .then(
                (value) => {
                    if (!isEmpty(value) && value.success === true) {
                        ...
                        const rows = this.updateTableRows(valueData.etId, value.data);
    ...
    updateTableRows = (id, item) => {
        ...
        const insertIndex = id === 0 ? 0 : rows.findIndex(item => item.etId === id);
    ...
    delete() {
        ...
        avrFetch(DELETE_URL.replace("{id}", id), {method: "DELETE",})
            ...
            .then(
                (value) => {
                    if (!isEmpty(value) && value.success === true) {
                        const rows = this.state.tableData.slice().filter(item => item.etId !== id);
                        this.setState(Object.assign({}, this.state, {
                            tableData: rows,
                            snackBar: Object.assign({}, this.state.snackBar, {
                                show: true,
