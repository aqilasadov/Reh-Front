...
class Kassa extends CoreComponent {
    constructor(props) {
        super(props);
        this.state = {
            processing: false,
    ...
    handleOnChange = name => data => {
        const val = (name === 'branch' || name === 'currency') ? data : data.target.value;
    ...
    handleTableAction = (id, action) => {
        ...
        if (action === 'add') {
            this.getMethod(BACKEND_URL + '/api/kassa/generatecode', this.processing, this.error, data => {
...
export default withStyles(styles)(connect(null, null)(Kassa));
